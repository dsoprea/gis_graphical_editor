...
- **Select All** / **Select None** — to the left of Split, Delete, and Undo (with a gap before those actions); check or clear every segment checkbox. Each button is enabled only when its action would change the current selection.
