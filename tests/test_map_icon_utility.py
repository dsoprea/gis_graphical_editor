...
def test_build_vcr_play_button_image_uses_requested_dimensions():
  image = gis_graphical_editor.map_icon_utility.build_vcr_play_button_image(38, 27)

  assert image.size == (38, 27)


def test_build_rewind_play_button_image_is_horizontally_flipped():
  forward_image = gis_graphical_editor.map_icon_utility.build_vcr_play_button_image(38, 27)
  rewind_image = gis_graphical_editor.map_icon_utility.build_rewind_play_button_image(38, 27)
  expected_rewind_image = forward_image.transpose(PIL.Image.FLIP_LEFT_RIGHT)

  assert rewind_image.tobytes() == expected_rewind_image.tobytes()
